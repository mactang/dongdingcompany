...
#import "MechanicalTariffViewController.h"
@interface MechanicalViewController ()<UITableViewDataSource,UITableViewDelegate>{
    UITableView  *_tableview;
}

@end

@implementation MechanicalViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.titleLabel.text = @"局域网维护";
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    [headerView titlelabel:headerarray[section] headerblock:^(UIButton *button) {
        MechanicalTariffViewController *mechanicaltariff = [[MechanicalTariffViewController alloc]init];
        [self.navigationController pushViewController:mechanicaltariff animated:YES];
    }];
    return headerView;
}
